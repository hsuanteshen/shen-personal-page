...
const blogBuild = await buildIndex(POSTS_DIR, 'blog.index.json', ({name, meta, body, nd})=>{
  ...
  // 乾淨的 topic slug / 顯示名
  const topicSlug = String(topicRaw).toLowerCase()
    .replace(/[^a-z0-9]+/g,'-')
    .replace(/^-+|-+$/g,'') || 'misc';
...
// --------- Blog Topics ---------
try {
  const items = blogBuild.items;
  const topicsMap = new Map();

  for (const p of items) {
    const key = p.topic?.slug || 'misc';
    const cur = topicsMap.get(key) || {
      slug: key,
      name: p.topic?.name || 'Misc',
      count: 0,
      cover: null,
      latestDate: null
    };
    ...
    if (!cur.latestDate || new Date(p.date||0) > new Date(cur.latestDate||0)) {
      cur.latestDate = p.date;
    }
  ...
  const topics = Array.from(topicsMap.values()).sort(
    (a,b) => (b.count - a.count) ||
             (new Date(b.latestDate||0) - new Date(a.latestDate||0))
  );

  await writeFile(path.join(OUT_DIR,'blog.topics.json'),
                  JSON.stringify(topics, null, 2), 'utf8');
